				...
				<template scope="data">
					<y-item :title="data.text">
						<div slot="foot" v-if="data.id === '2'" class="edit-number-container">
							<y-number v-model="num" :min="1" :max="500" :disabled="dataway.id === '1'"></y-number>
							<span class="edit-consulatway-time">悠然币/永久</span>
						</div>
					</y-item>
				</template>
			</y-check-group>
...
export default {
	...
	data() {
		return {
			num: 1,
			dataway: [{ text: '免费', id: '1' }, { text: '收费', id: '2' }],
			currItem: null,
		}
	},
	watch: {
	...
	created() {
		this.$http.get(`/services/app/v1/coterie/info/single/${this.$route.params.coterieId}`).then(res => {
			...
			if (this.coterieData.joinFee === 0) {
				...
				this.num = this.coterieData.joinFee / 100;
				this.dataway[0].checked = false;
				this.dataway[1].checked = true;
			}
			this.currItem = this.dataway[this.coterieData.joinFee === 0 ? 0 : 1];
		});
	},
	methods: {
		getItem(item) {
		...
		submit() {
			let params = {
				joinFee: this.currItem['id'] === '1' ? 0 : this.num * 100,
			}
			this.$http.put(`/services/app/v1/coterie/info/single/${this.$coterie.coterieId}`, params).then(res => {
				if (res.data.code === '200') {
					Toast("修改成功！")
					this.$coterie.joinFee = params.joinFee;
				} else {
					Toast(res.data.msg)
				}
