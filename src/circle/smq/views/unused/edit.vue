<template>
	<div class="unused-release">
		<y-nav :title="$R('unuse-release')" :leftText="$R('cancel')" :beforeBack="goBack" :showLeftArrow="false">
			<span slot="nav-right">
				<y-publish-button>{{$R('unuse-publish')}}</y-publish-button>
			</span>
		</y-nav>
		...
		<y-input :maxlength="30" :minlength="4" :placeholder="$R('edit-contact')" v-model="vm.contactInformation"></y-input>
		<y-list>
			<y-item :title="$R('choose-location')" to='/unused/edit-city' v-model="vm.address"></y-item>
			<y-item :title="$R('choose-fineness')" to="/unused/edit-fineness" v-model="vm.damageStatustext"></y-item>
			<y-item :title="$R('choose-category')" to="/unused/edit-category" v-model="vm.classifyItemName"></y-item>
		</y-list>
		<y-editor v-model="vm.contentSource" :text-max-length="10000" :img-max-length="10" :placeholder="$R('transaction-details')" ref="nativeEditor"></y-editor>
	</div>
</template>
<script>
...
export default {
	...
	methods: {
		initData() {
			this.$localStore.getOrSet('petDeta', null, this.vm).then(data => {
				this.vm = data;
			})
		...
		validate() {
			...
			this.postData = {
				...summaryData,
				moduleEnum: '10133',
				classifyItemName: this.vm.classifyItemName,
		...
		publish() {
			this.$http.post('/services/app/v1/idlemarket/publish', this.postData).then(response => {
				let data = response.data;
				if (data.code === "200") {
					Toast(this.$R("publish-success"));
					this.publishSuccess('/unused/index');
					this.$localStore.remove('petDeta')
				} else {
