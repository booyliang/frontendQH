	...
	<div id="app" class="jkys-attestation-info">
		...
		<div class="">
			<y-list>
				...
				<y-item>
					<input v-model="tags" type="text" placeholder="请输入擅长领域(如鼻炎，最多7个字，逗号隔开)" />
				</y-item>
				<y-item>
					<y-input v-model="vm.data.workCity" :maxlength="15" placeholder="请输入工作城市(如北京市)" :show-text-length-info="false"></y-input>
				</y-item>
				<y-item>
...
export default {
	...
	data() {
		return {
			...
			imgsrc3: '',
			tags: '',
			//	permit: "/shoot/permit/" + this.$route.params.id,
			//	handCard: "/shoot/hand-card/" + this.$route.params.id,
			//	idCard: "/shoot/id-card/" + this.$route.params.id
		}
	...
	created() {
		...
		this.tags = this.vm.data.speciality;
		console.log(this.vm);
	},
	watch: {
		tags(value, obkey) {
			...
			if (this.tags) {
				this.vm.data.speciality = this.tags
			}
		}
	},
	methods: {
		...
		onClick() {
			...
			if (!postData.organization) {
				Toast('单位组织信息不能为空');
				return false;
			} else if (!postData.occupation) {
				Toast('职位称号不能为空');
				return false;
			} else if (!postData.speciality) {
				Toast('擅长领域不能为空');
				return false;
			} else if (!postData.workCity) {
				Toast('工作城市不能为空');
				return false;
			} else if (!/^[\u4e00-\u9fa5]+$/gi.test(postData.workCity)) {
				Toast('请输入纯汉字城市名');
				return false;
			...
			let Murl = postData.id ? '/services/app/v1/doctorAuthentication/single' : '/services/app/v1/doctorAuthentication/single'
