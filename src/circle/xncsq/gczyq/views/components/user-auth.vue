  ...
  <div class="authentication" v-if="userData.authStatus && userData.authStatus === 1" >
		<router-link tag="div" class="authentication_item" :to="`/shoot/ad/${ userData.userId }`">
			...
			<div class="authentication_item-info">
				{{authenticationInfo.organization}}  {{authenticationInfo.occupation }}
	...
	methods: {
		getData() {
			if (!this.userData || !this.userData.userId) 
				return;
			this.$http.get(`/services/app/v1/doctorAuthentication/getAuthenticationInfoByPersonal/${this.userData.userId}`) 
		.then((res) => {
			this.authenticationInfo = res.data.data;
			this.tags = this.authenticationInfo.speciality.split(',');
		}).catch((err) => {
