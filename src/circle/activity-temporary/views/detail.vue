<template>
	<div class="temporary_activity-detail">
		...
		<div class="temporary_activity-content">
			  <p class="temporary_activity-count"><span>编号：{{ detailData.voteNo }}</span> <label>票数：<i>{{ detailData.voteCount }}</i></label></p>
			  <p class="temporary_activity-name"><span>{{ detailData.title }}</span> <label>{{ detailData.createDate | recentTime }}</label></p>
			  <p class="temporary_activity-school"><span>{{ detailData.schoolName }}</span></p>
			   <p class="temporary_activity-img" v-if="detailData.coverPlan">
					<img :src="detailData.coverPlan | imageResize(6)" alt="">
				</p>
			  ...
			  <div class="temporary_activity-vote">
				   ...
				   <router-link class="go_index_button" :to="{name: 'index'}">
					   投票<br>列表
				   </router-link>
			  </div>
		</div>
	...
	export default {
		...
		methods: {
			...
			voted() {
				...
				this.$http.post(`/services/app/v1/temp/activity/vote/opus`, {candidateId: this.detailData.id}).then(response => {
					this.$indicator.close();
					let resData = response.data;
					if (resData.code === '200') {
						this.$eventBus.$emit('voteToast', 'success');
						this.detailData.voteFlag ++;
