...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.Phone = this.vm.data.linkmanPhone;
		},
		computed: {
			beforeDestroy() {
		...
		methods: {
			...
			async submit() {
				...
				if (length <= 10 || !/^1[3|4|5|7|8|9]\d{9}$/.test(this.Phone)) {
					Toast(this.$R('error-phone')) 
					return;
				} 
				if (!this.verifyCode) {
				...
				if (httpUrl.data.data) {
					this.vm.data.linkmanPhone = this.Phone;
					this.$router.back();
				} else {
